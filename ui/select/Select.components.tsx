...
>(({ className, children, ...props }, ref) => (
  ...
  >
    ...
    <SelectPrimitive.Icon asChild>
      <ChevronDownIcon className='h-5 w-5 flex-shrink-0 stroke-2 text-slate-500 transition group-data-[state=open]:rotate-180 group-data-[disabled]:text-slate-400' />
...
>(({ className, children, position = 'popper', ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={cn(
        'relative z-select max-h-60 min-w-32 overflow-y-auto bg-white',
    ...
    >
      <SelectPrimitive.Viewport
        className={cn({
          'h-radix-select-trigger w-full min-w-radix-select-trigger overflow-hidden rounded border-1.5 border-slate-200':
            position === 'popper'
        })}
      >
        {children}
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Item
    ...
    {...props}
  >
    <span className='absolute right-2 mr-2 flex h-5 w-5 items-center justify-center stroke-2'>

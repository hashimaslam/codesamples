...
>(({ className, ...props }, ref) => (
  <td
    ref={ref}
    className={cn(
      'p-4 align-middle text-sm text-gray-800 [&:has([role=checkbox])]:pr-0',
      className
    )}
    {...props}
  />
      ...
      >
        {actions.map(({ label, icon: Icon, onClick, highlight }) => (
          <Popover.PopoverClose key={label} asChild>
            ...
            >
              <Icon className='h-4 w-4 stroke-2.5 text-slate-800' />
...
const TablePinAction = forwardRef<SVGSVGElement, TablePinActionProps>(
  ({ className, unpin, ...props }, ref) => (
    ...
    >
      <PinRightIcon
        ref={ref}
        className={cn('h-4 w-4 stroke-2.5', className)}
        {...props}
      />
    </button>
  )

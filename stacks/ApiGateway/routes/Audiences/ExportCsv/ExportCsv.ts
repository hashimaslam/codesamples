...
export class ExportCsv extends Construct {
  constructor(scope: Construct, id: string, private props: { api: IRestApi }) {
    ...
    const fnBg = new TypescriptFunction(
      this,
      `${id}-ExportCsv.background.lambda.ts`,
      {
    ...
    const fn = new TypescriptFunction(this, `${id}-ExportCsv.startJob`, {
      entry: path.resolve(__dirname, "ExportCsv.startJob.lambda.ts"),
      description: `Endpoint: Get /audiences/csv`,
      timeout: Duration.seconds(10),
      environment: {
        ARN_LAMBDA_FOR_BACKGROUND_EXPORTING_AUDIENCES: fnBg.functionArn,
      },
      initialPolicy: [
        new PolicyStatement({
